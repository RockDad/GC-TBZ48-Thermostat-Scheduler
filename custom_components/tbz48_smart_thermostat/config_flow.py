"""Configure the custom configuration tbz48_smart_thermostat."""
...
class TBZ48ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input:
            ...
            tolerance_max = (
                DEFAULT_TOLERANCE_MAX_F
                if user_input["temperature_unit"] == UNIT_FARENHEIT
                else DEFAULT_TOLERANCE_MAX_C
            )
            self._data["tolerance_max"] = tolerance_max
            sensor_list = user_input["sensor_filter"]
            self._data["current_sensor"] = (
                sensor_list[0] if sensor_list else DEFAULT_SENSOR
            )
        ...
        return self.async_show_form(
            step_id="user", data_schema=self._build_user_schema(sensors)
        )
    ...
    def _build_user_schema(self, sensors):
        ...
        return vol.Schema(
            {
                vol.Required("temperature_unit", default=UNIT_CELSIUS): vol.In(
                    [UNIT_CELSIUS, UNIT_FARENHEIT]
                ),
                ...
                vol.Required(
                    "sensor_filter", default=[default_sensor]
    ...
    async def async_step_mf_config(self, user_input=None):
        ...
        for i in range(1, 5):
            ...
            schema[vol.Required(f"m_f_temperature_{i}", default=temp_default)] = (
                vol.All(vol.Coerce(float), vol.Range(min=temp_min, max=temp_max))
            )
            schema[vol.Required(f"m_f_sensor_{i}", default=default_sensor)] = vol.In(
                sensors
            )
    ...
    async def async_step_ss_config(self, user_input=None):
        ...
        for i in range(1, 5):
            ...
            schema[vol.Required(f"s_s_temperature_{i}", default=temp_default)] = (
                vol.All(vol.Coerce(float), vol.Range(min=temp_min, max=temp_max))
            )
            schema[vol.Required(f"s_s_sensor_{i}", default=default_sensor)] = vol.In(
                sensors
            )
    ...
    async def _get_temp_sensor_options(self):
        try:
            ...
            sensors = {
                ...
                if e.domain == "sensor"
                and (
                    "temperature" in (e.device_class or "")
                    or "temperature" in e.entity_id
                )
            }
            if not sensors:
...
class TBZ48OptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input=None):
        ...
        all_sensors = {
            ...
            if e.domain == "sensor"
            and (
                "temperature" in (e.device_class or "") or "temperature" in e.entity_id
            )
        }
        ...
        if user_input:
            self._data["sensor_filter"] = user_input["sensor_filter"]
            self._data["current_sensor"] = (
                user_input["sensor_filter"][0]
                if user_input["sensor_filter"]
                else DEFAULT_SENSOR
            )
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema(
                {
                    vol.Required("sensor_filter", default=selected): cv.multi_select(
                        all_sensors
                    ),
                }
            ),
