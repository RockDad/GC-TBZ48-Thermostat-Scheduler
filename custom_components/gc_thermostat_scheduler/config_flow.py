...
class T6ProgramConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input:
            ...
            self._data["heat_setpoint_range"] = user_input["heat_setpoint_range"]  #ejg
            self._data["cool_setpoint_range"] = user_input["cool_setpoint_range"], #ejg
            _LOGGER.debug("User input received in user step: %s", user_input)
    ...
    def _build_user_schema(self, sensors):
        ...
        return vol.Schema({
            ...
            vol.Required("sensor_filter", default=[default_sensor]): cv.multi_select(sensors), #ejg - added comma at EOL
            vol.Required("heat_setpoint_range") : vol.Coerce(float), #ejg
            vol.Required("cool_setpoint_range") : vol.Coerce(float) #ejg
        })
